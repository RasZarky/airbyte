...
A lightweight Airbyte source connector that extracts search engine results using Bright Data's SERP API.
...
# Test connection
poetry run source-brightdata-serp check --config secrets/config.json

py -3.11 -m poetry run source-brightdata-serp check --config secrets/config.json


# Discover streams and schema
poetry run source-brightdata-serp discover --config secrets/config.json

py -3.11 -m poetry run source-brightdata-serp discover --config secrets/config.json


# Read data
poetry run source-brightdata-serp read --config secrets/config.json --catalog integration_tests/configured_catalog.json

py -3.11 -m poetry run source-brightdata-serp read --config secrets/config.json --catalog integration_tests/configured_catalog.json


poetry run pytest unit_tests

airbyte-ci connectors --name=source-brightdata-serp build